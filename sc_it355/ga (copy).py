...
def genetic_algo():
    ...
    for i in range(pop_size):
        chromosome = []
        for j in range(ch_num):  # here we randomly generate chromosomes
            num = np.random.random(1)  # adds 1 as a gene to the chromsome
            if num > 0.5:
                chromosome.append(1)
            else:
                chromosome.append(0)
        ch_matrix.append(chromosome)
    ch_matrix = np.array(ch_matrix)  # making an array out of the chromosomes made
    for i in range(10):  # genetic algo is applied 10 times
